...
export class AdminSupport implements OnInit, OnDestroy {
  ...
  // ── Category helpers (mirrors support-view.ts) ────────────────────────────
  private readonly catMeta: Record<string, { label: string; icon: string; cssClass: string }> = {
    VIREMENT: { label: 'Virement', icon: 'fa-solid fa-right-left', cssClass: 'purple' },
    COMPTE: { label: 'Compte', icon: 'fa-solid fa-credit-card', cssClass: 'blue' },
    CARTE: { label: 'Carte', icon: 'fa-solid fa-id-card', cssClass: 'navy' },
    PRET: { label: 'Prêt', icon: 'fa-solid fa-hand-holding-dollar', cssClass: 'amber' },
    PLACEMENT: { label: 'Placement', icon: 'fa-solid fa-chart-line', cssClass: 'green' },
    SECURITE: { label: 'Sécurité', icon: 'fa-solid fa-shield-halved', cssClass: 'red' },
    AUTRE: { label: 'Autre', icon: 'fa-solid fa-circle-question', cssClass: 'grey' },
  };

  getCatLabel(cat: string): string { return this.catMeta[cat]?.label ?? cat; }
  getCatIcon(cat: string): string { return this.catMeta[cat]?.icon ?? 'fa-solid fa-circle-question'; }
  getCatClass(cat: string): string { return this.catMeta[cat]?.cssClass ?? 'grey'; }
}